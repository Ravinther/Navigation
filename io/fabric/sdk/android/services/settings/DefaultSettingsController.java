...
class DefaultSettingsController implements SettingsController {
    ...
    public SettingsData loadSettingsData(SettingsCacheBehavior cacheBehavior) {
        ...
        try {
            ...
            Fabric.getLogger().m1456e("Fabric", "Unknown error while loading Crashlytics settings. Crashes will be cached until settings can be retrieved.", e);
        }
        return toReturn;
    }
            ...
            if (settingsJson != null) {
                ...
                if (settingsData != null) {
                    ...
                    if (SettingsCacheBehavior.IGNORE_CACHE_EXPIRATION.equals(cacheBehavior) || !settingsData.isExpired(currentTimeMillis)) {
                        ...
                        Fabric.getLogger().m1453d("Fabric", "Returning cached settings.");
                        return toReturn;
                    }
                    Fabric.getLogger().m1453d("Fabric", "Cached settings have expired.");
                    return null;
                }
                Fabric.getLogger().m1456e("Fabric", "Failed to transform cached settings data.", null);
                return null;
            }
            Fabric.getLogger().m1453d("Fabric", "No cached settings data found.");
            return null;
        } catch (Exception e) {
            Fabric.getLogger().m1456e("Fabric", "Failed to get cached settings", e);
            return null;
        }
    }

    private void logSettings(JSONObject json, String message) throws JSONException {
        Fabric.getLogger().m1453d("Fabric", message + json.toString());
    }
