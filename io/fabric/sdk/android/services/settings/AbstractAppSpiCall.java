...
abstract class AbstractAppSpiCall extends AbstractSpiCall {
    ...
    public boolean invoke(AppRequestData requestData) {
        ...
        Fabric.getLogger().m1453d("Fabric", "Sending app info to " + getUrl());
        if (requestData.icon != null) {
            Fabric.getLogger().m1453d("Fabric", "App icon hash is " + requestData.icon.hash);
            Fabric.getLogger().m1453d("Fabric", "App icon size is " + requestData.icon.width + "x" + requestData.icon.height);
        }
        int statusCode = httpRequest.code();
        Fabric.getLogger().m1453d("Fabric", ("POST".equals(httpRequest.method()) ? "Create" : "Update") + " app request ID: " + httpRequest.header("X-REQUEST-ID"));
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest request, AppRequestData requestData) {
        ...
        if (requestData.icon != null) {
            ...
            try {
                ...
                Fabric.getLogger().m1456e("Fabric", "Failed to find app icon with resource ID: " + requestData.icon.iconResourceId, e);
            } finally {
