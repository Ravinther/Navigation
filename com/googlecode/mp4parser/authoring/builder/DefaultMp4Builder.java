...
public class DefaultMp4Builder {
    ...
    protected Box createEdts(Track track, Movie movie) {
        ...
        for (Edit edit : track.getEdits()) {
            entries.add(new Entry(elst, ((long) edit.getSegmentDuration()) * movie.getTimescale(), (edit.getMediaTime() * track.getTrackMetaData().getTimescale()) / edit.getTimeScale(), edit.getMediaRate()));
        }
        ...
        for (Map.Entry<String, List<GroupEntry>> sg2 : groupEntryFamilies.entrySet()) {
            ...
            for (int i = 0; i < track.getSamples().size(); i++) {
                ...
                for (int j = 0; j < ((List) sg2.getValue()).size(); j++) {
                    GroupEntry groupEntry = (GroupEntry) ((List) sg2.getValue()).get(j);
                    if (Arrays.binarySearch((long[]) track.getSampleGroups().get(groupEntry), (long) i) >= 0) {
                        index = j + 1;
                    }
                }
    ...
    protected void createStco(Track track, Movie movie, Map<Track, int[]> chunks, SampleTableBox stbl) {
        ...
        for (int i = 0; i < tracksChunkSizes.length; i++) {
            ...
            for (Track current : movie.getTracks()) {
                ...
                while (true) {
                    if (((long) j) < ((long) chunkSizes[i]) + firstSampleOfChunk) {
