...
public class CencMp4TrackImplImpl extends Mp4TrackImpl implements CencEncryptedTrack {
    ...
    public CencMp4TrackImplImpl(String name, TrackBox trackBox, IsoFile... fragments) throws IOException {
        ...
        if ($assertionsDisabled || (schm != null && (schm.getSchemeType().equals("cenc") || schm.getSchemeType().equals("cbc1")))) {
                ...
                for (MovieFragmentBox movieFragmentBox : ((Box) trackBox.getParent()).getParent().getBoxes(MovieFragmentBox.class)) {
                    for (TrackFragmentBox traf : movieFragmentBox.getBoxes(TrackFragmentBox.class)) {
                        if (traf.getTrackFragmentHeaderBox().getTrackId() == trackId) {
                            Container base;
                            long baseOffset;
                            tenc = (TrackEncryptionBox) Path.getPath(trackBox, "mdia[0]/minf[0]/stbl[0]/stsd[0]/enc.[0]/sinf[0]/schi[0]/tenc[0]");
                            ...
                            if ($assertionsDisabled || saio != null) {
                                ...
                                if (!$assertionsDisabled) {
                                    if (saioOffsets.length != traf.getBoxes(TrackRunBox.class).size()) {
                                        throw new AssertionError();
                                    }
                                }
                                if ($assertionsDisabled || saiz != null) {
                                    ...
                                    while (true) {
                                        ...
                                        for (j = sampleNo; j < sampleNo + numSamples; j++) {
                                            length2 += (long) saiz.getSize(j);
                                        }
                                        ByteBuffer trunsCencSampleAuxData = base.getByteBuffer(baseOffset + offset, length2);
                                        for (j = sampleNo; j < sampleNo + numSamples; j++) {
                                            int auxInfoSize = saiz.getSize(j);
                                            long j2 = (long) auxInfoSize;
                                            this.sampleEncryptionEntries.add(parseCencAuxDataFormat(tenc.getDefaultIvSize(), trunsCencSampleAuxData, j2));
                                        }
                                        sampleNo += numSamples;
                                        i++;
                                    }
            ...
            saizSaioPair = new FindSaioSaizPair((Container) Path.getPath(trackBox, "mdia[0]/minf[0]/stbl[0]")).invoke();
            saio = saizSaioPair.saio;
            saiz = saizSaioPair.saiz;
            Container topLevel = ((MovieBox) trackBox.getParent()).getParent();
            length = saio.getOffsets().length;
                ...
                while (true) {
                    ...
                    if (i < r0) {
                        ...
                        while (true) {
                            ...
                            long auxInfoSize2 = (long) saiz.getSize(currentSampleNo + j);
                            this.sampleEncryptionEntries.add(parseCencAuxDataFormat(tenc.getDefaultIvSize(), chunksCencSampleAuxData, auxInfoSize2));
                            j++;
                        }
