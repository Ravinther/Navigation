...
public class Beta extends Kit<Boolean> implements DeviceIdentifierProvider {
    ...
    protected Boolean doInBackground() {
        Fabric.getLogger().m1453d("Beta", "Beta kit initializing...");
        Context context = getContext();
        IdManager idManager = getIdManager();
        if (TextUtils.isEmpty(getBetaDeviceToken(context, idManager.getInstallerPackageName()))) {
            Fabric.getLogger().m1453d("Beta", "A Beta device token was not found for this app");
            return Boolean.valueOf(false);
        }
        Fabric.getLogger().m1453d("Beta", "Beta device token is present, checking for app updates.");
    ...
    private String getBetaDeviceToken(Context context, String installerPackageName) {
        if (isAppPossiblyInstalledByBeta(installerPackageName, VERSION.SDK_INT)) {
            Fabric.getLogger().m1453d("Beta", "App was possibly installed by Beta. Getting device token");
            try {
                ...
                Fabric.getLogger().m1456e("Beta", "Failed to load the Beta device token", e);
                return null;
            }
        }
        Fabric.getLogger().m1453d("Beta", "App was not installed by Beta. Skipping device token");
        return null;
    }
    ...
    private BuildProperties loadBuildProperties(Context context) {
        ...
        try {
            ...
            if (buildPropsStream != null) {
                ...
                Fabric.getLogger().m1453d("Beta", buildProps.packageName + " build properties: " + buildProps.versionName + " (" + buildProps.versionCode + ")" + " - " + buildProps.buildId);
            }
            if (buildPropsStream != null) {
                try {
                    buildPropsStream.close();
                } catch (IOException e) {
                    Fabric.getLogger().m1456e("Beta", "Error closing Beta build properties asset", e);
                }
            }
        } catch (Exception e2) {
            Fabric.getLogger().m1456e("Beta", "Error reading Beta build properties", e2);
            if (buildPropsStream != null) {
                try {
                    buildPropsStream.close();
                } catch (IOException e3) {
                    Fabric.getLogger().m1456e("Beta", "Error closing Beta build properties asset", e3);
                }
            }
        } catch (Throwable th) {
            if (buildPropsStream != null) {
                try {
                    ...
                    Fabric.getLogger().m1456e("Beta", "Error closing Beta build properties asset", e32);
                }
            }
        }
        return buildProps;
