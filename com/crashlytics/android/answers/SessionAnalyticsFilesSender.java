...
class SessionAnalyticsFilesSender extends AbstractSpiCall implements FilesSender {
    ...
    public boolean send(List<File> files) {
        ...
        Fabric.getLogger().m1453d("Answers", "Sending " + files.size() + " analytics files to " + getUrl());
        int statusCode = httpRequest.code();
        Fabric.getLogger().m1453d("Answers", "Response code for analytics file send is " + statusCode);
